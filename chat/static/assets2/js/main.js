...
(function() {
  ...
  */
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      const dropdownLinks = document.querySelectorAll(".nav-content a"); // Get all dropdown items
      const currentPath = window.location.pathname;
  
      navLinks.forEach((link) => {
        const linkPath = link.getAttribute("href");
  
        // Ignore links with no href or empty href
        if (!linkPath) return;
  
        // If the main link is active, highlight it
        if (currentPath === linkPath) {
          link.classList.remove("collapsed");
          link.classList.add("active");
  
          // If it's a dropdown child, activate the parent dropdown too
          let parentNav = link.closest(".nav-content");
          if (parentNav) {
            let parentLink = parentNav.previousElementSibling;
